...
function networkUp() {
  ...
  export BYFN_CA_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org1.example.com/ca && ls *_sk)
  echo "BYFN_CA_PRIVATE_KEY="$BYFN_CA_PRIVATE_KEY
  ...
  if [ "$CONSENSUS_TYPE" == "kafka" ]; then
    sleep 20
    echo "Sleeping 70s to allow $CONSENSUS_TYPE cluster to complete booting"
    sleep 70
  fi
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    ...
    docker-compose $COMPOSE_FILES up -d ca.example.com orderer0.example.com
